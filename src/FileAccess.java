...
public class FileAccess {
  ...
  public static Workouts loadWorkouts() {
    ...
    try { 
      Scanner scanner = new Scanner(new File("data/" +Config.WORKOUTFILE));
  ...
  public static EnumMap<Config.MuscleGroup, ArrayList<Config.Muscle>> loadFormats() {
    ...
    try {
    	Scanner scanner = new Scanner(new File("data/" +Config.WORKOUTFORMATFILE));
    	while (scanner.hasNextLine()){
    		String line = scanner.nextLine();
        String[] fields = line.split(",");
        
        Config.MuscleGroup msgroup = Config.MuscleGroup.valueOf(fields[0]);
        ArrayList<Config.Muscle> aryList = new ArrayList<Config.Muscle>();
        
        for(int i = 1; i < fields.length; i++){
        	Config.Muscle muscle = Config.Muscle.valueOf(fields[i]);
        	aryList.add(muscle);
        }
        retval.put(msgroup, aryList);
    	}
    }
    catch (FileNotFoundException e){
    	System.out.println("Unable to find workoutformats file. Is itin the same directory as the xecutabel?\nError: " + e.toString());
    }
